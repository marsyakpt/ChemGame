...
# --------------------- MATERI ---------------------
    ...
    if st.session_state.materi_topik_organik is None:
        ...
        if topik == "hidrokarbon":
            ...
            - a. Dengan air brom (Br₂ dalam CCl₄)
               -  Alkena (tak jenuh): larutan brom hilang warnanya (dekolorisasi)
               -  Alkana & aromatik: tidak bereaksi (tetap berwarna coklat)
...
# ---------------------- GAME ----------------------
    ...
    if st.session_state.selesai:
        ...
        emoji_medal = ["🥇", "🥈", "🥉", "🎖️", "🏅"]
        # Warna gelap+neon yang kontras di dark mode
        ...
        for i, entry in enumerate(sorted_leaderboard):
            ...
            st.markdown(
                f"""
                <div style='
                    background-color:{warna_bg[i]};
                    padding:12px;
                    border-radius:10px;
                    margin-bottom:8px;
                    border: 1px solid #EEEEEE;
                '>
                    <span style='font-size:18px; color:{warna_teks[i]}; font-weight:bold;'>
                        {emoji_medal[i]} {nama} — {skor} poin
                    </span>
                </div>
                """,
